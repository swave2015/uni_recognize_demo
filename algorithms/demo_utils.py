...
def resize_or_pad(input_img, target_size):
    """
    ...
    """    
    if input_img.shape[0] > target_size or input_img.shape[1] > target_size:
        img_resized = cv2.resize(input_img, (target_size, target_size))

        return img_resized
    else:
        top = (target_size - input_img.shape[0]) // 2
        bottom = target_size - input_img.shape[0] - top
        left = (target_size - input_img.shape[1]) // 2
        right = target_size - input_img.shape[1] - left
        img_padded = cv2.copyMakeBorder(input_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0, 0, 0])
